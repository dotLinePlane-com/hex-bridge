"""
MCP24 IPC Bridge �?real-time COM3 via serial-monitor MCP tools

�?test_uart_mcp.py 配合使用:
  - 测试脚本只打开 COM4 (pyserial, UBCP)
  - COM3 操作通过文件 IPC 转发�?Kilo
  - Kilo 调用 serial-monitor-mcp_* 工具
  - MCP Serial Monitor UI 实时显示所�?COM3 数据�?
IPC 协议:
  请求: %TEMP%/mcp24_req.txt  (JSON, 单行)
  响应: %TEMP%/mcp24_res.txt  (JSON, 单行)
  标记: %TEMP%/mcp24_replay.jsonl  (JSONL, 追加)

�?__MCP24_ACTIVE__=1 时启�?IPC 模式�?否则回退 pyserial 直连 (兼容独立运行)�?"""
...
class MCP24IPCBridge:
    ...
    def __init__(self, port="COM3", baud=115200):
        ...
        if self._is_active:
            ...
            # Request Kilo to open COM3 via MCP
            r = self._ipc("mcp_open", {"port": port, "baudRate": baud})
            if r and r.get("ok"):
                print(f"[MCP24] COM3 opened via MCP", file=sys.stderr)
        else:
    ...
    def marker(self, text):
        ...
        if self._is_active:
            # Also send as readable text on COM3 via MCP
