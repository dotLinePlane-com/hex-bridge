...
| 方向 | 路径 |
|:---|:---|
| MCP 命令 | Serial Monitor (COM4) → HEX-Bridge |
| MCP 响应/事件 | HEX-Bridge → Serial Monitor (COM4) |
| HEX-Bridge TCP 发送 | COM4 → MCP 命令 → HEX-Bridge → Ethernet → Network Monitor (PC) |
| HEX-Bridge TCP 接收 | Network Monitor (PC) → Ethernet → HEX-Bridge → MCP 事件 → COM4 |
| HEX-Bridge TCP Server 接受 | Network Monitor Client → Ethernet → HEX-Bridge → TCP_ACCEPT 事件 → COM4 |
| DNS 解析 | COM4 → MCP → HEX-Bridge → DNS 服务器 → MCP → COM4 |
...
| MCP 通信口 | COM4, UART1, 921600 bps, 8N1 |
...
2. 网线已插入, 链路 UP (可通过 COM5 日志确认 `Ethernet Link Up`)
...
| 2. 发送 MCP 命令 | **Serial Monitor** (COM4) | 通过 UBCP 命令控制 HEX-Bridge 执行网络操作 |
| 3. 验证 MCP 响应 | **Serial Monitor** (COM4) | 读取 UBCP 响应/事件帧 |
...
> 测试步骤中标注 `[MCP NM]` 的操作在 Kilo Agent 中执行，`[COM4]` 的操作通过 Serial Monitor 发送 UBCP 命令。
...
| **涉及工具** | Serial Monitor (COM4) + MCP Network Monitor |
...
2. **[COM4] 获取 HEX-Bridge IP**
   ```
   发送 NET_STATUS(0x00) → 记录 IpAddr
   ```

3. **[COM4] 获取 PC 本机 IP (用于 HEX-Bridge 连接)**
   ```bash
   ipconfig → 记录 PC IP
   ```

4. **[COM4] TCP_CLIENT_CONNECT 到 PC**
   ```
...
9. **[COM4] 等待 TCP_DISCONNECT_EVENT**
   ```
...
3. **[COM4] 等待 2 次 TCP_ACCEPT 事件** → 记录 C1, C2
...
3. **[COM4] TCP_ACCEPT 拒绝** (Decision=0x01)
...
4. **[COM4] 等待 UDP_RECV 事件**
   ```
   → 预期: Handle=SH, SrcIP=PC IP, SrcPort=已分配, Data="UDP HELLO"
   ```

5. **[COM4] UDP_SERVER_SEND 回复到 MCP NM**
   ```
...
4. **[MCP NM] 验证**
   ...
   ```
5. **交换方向**: MCP NM 发送 1024 字节, COM4 等待 TCP_RECV 验证完整
6. 清理
...
3. **[COM4] 等待 2 次 TCP_ACCEPT 事件** → 记录 CH_A, CH_B
...
6. **[COM4] 等待 TCP_DISCONNECT_EVENT(CH_A)** → 收到
...
3. **[COM4] 等待 2 次 WS_ACCEPT 事件** → 记录 CH_A, CH_B
...
6. **[COM4] 等待 WS_DISCONNECT_EVENT(CH_A)** → 收到
...
| **测试方法** | 设备上电后监听 COM4, 等待 DEV 侧主动上报的事件帧 |
...
2. HEX-Bridge 断电重启
3. 监听 COM4 - 预期收到 LINK_UP 事件 (EventType=0x01)
...
2. 监听 COM4 记录所有 NET_LINK_EVENT 事件
...
2. 监听 COM4, 等待 TCP_DISCONNECT_EVENT
...
3. 测试脚本通过 COM4 发送 TCP_ACCEPT **命令** (DIR=0) 确认
...
4. 通过 COM4 快速连续 TCP_SEND 大数据, 直到发送缓冲区满
...
3. 监听 COM4 — 不应收到 WS_RECV 事件 (Ping/Pong 在设备侧内部控制, 不上报给 UBCP)
...
#     port="COM4" data="<UBCP TCP_CLIENT_CONNECT 帧>" format="hex"
#
# 步骤 3: 双向收发验证
