...
| 时钟模式 | `EMAC_CLK_OUT` (PHY 提供 REF_CLK) |
| SMI 管理 | MDC=GPIO 23, MDIO=GPIO 18 |
| PHY 复位 | GPIO 5 (低有效, 外接 10kΩ 下拉) |
| PHY 地址 | 0x00 (默认, 由 PHYAD0 Pin=下拉 设定) |
...
固件启动:   app_main() → eth_hw_init() → LAN8720 PHY 初始化流程
              │
              ├─ 1. gpio_set_level(GPIO_NUM_5, 0)  // 确保复位
              ├─ 2. vTaskDelay(pdMS_TO_TICKS(20))   // 等待 20ms
              ├─ 3. gpio_set_level(GPIO_NUM_5, 1)   // 释放复位
              ├─ 4. vTaskDelay(pdMS_TO_TICKS(100))  // 等待 PHY 稳定
              │
              ├─ 5. esp_eth_mac_new_esp32(&mac_config, &mac)
              ├─ 6. eth_phy_new_lan8720(&phy_config, &phy)
              │
              ├─ 7. esp_eth_config_t eth_config = ETH_DEFAULT_CONFIG(mac, phy)
              ├─ 8. esp_eth_driver_install(&eth_config, &eth_handle)
              │
              ├─ 9. esp_netif_init()
              ├─10. esp_netif_create_default_eth(&eth_netif)
              ├─11. esp_eth_start(eth_handle)    // 启动以太网
              │
              └─12. dhcp_event_callback 等待 DHCP 获取 IP
```
...
| 参数 | 值 | 说明 |
|:---|:---|:---|
| `phy_addr` | `0x00` | PHY 地址 |
| `reset_gpio_num` | `GPIO_NUM_5` | 软件复位引脚 (低有效) |
| `clock_config.rmii.clock_mode` | `EMAC_CLK_OUT` | PHY 输出 50MHz 至 GPIO 0 |
| `clock_config.rmii.clock_gpio` | `EMAC_CLK_OUT_GPIO` (GPIO 0) | ESP-IDF 内部宏自动匹配 |
| `mdc_gpio_num` | `GPIO_NUM_23` | SMI 管理时钟 |
| `mdio_gpio_num` | `GPIO_NUM_18` | SMI 管理数据 |
| `smi_mdc_gpio_num` | `GPIO_NUM_23` | MAC 配置中 MDC |
| `smi_mdio_gpio_num` | `GPIO_NUM_18` | MAC 配置中 MDIO |
