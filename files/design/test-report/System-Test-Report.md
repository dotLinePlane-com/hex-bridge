...
| 已实现命�?| PING (0x00), GET_INFO (0x01), GET_CONFIG (0x02), SET_CONFIG (0x03), RESET (0x04), FLOW_CONTROL (0x05), SYS_BOOT_EVENT (0x06), GET_TOPOLOGY (0x07) |
| 测试用例�?| 17 (SYS-01~SYS-20, 排除 SYS-08 重复编号) |
| 测试断言�?| 81 |
...
|:---|:---|
| ESP-IDF 版本 | v6.0.1 |
| 编译�?| xtensa-esp-elf-gcc (esp-14.2.0_20240906) |
...
|:---|:---|
| ESP-IDF 版本 | v6.0.1 |
| 编译�?| xtensa-esp-elf-gcc (esp-15.2.0_20251204) |
...
```
┌──────────────�?                     ┌─────────────────�?�?  PC (COM4) │── MCP/UBCP ────────→│   HEX-Bridge     �?�? 测试客户�?  �?  921600 bps        �?  ESP32          �?�?             │←── MCP/UBCP ────────�?  UART1 (GP4/34) �?└──────────────�?                     └─────────────────�?COM5: ESP32 调试输出 + 烧录 (UART0, 115200 bps)
```
...
| SYS-06 | RESET + SYS_BOOT_EVENT | 0x04�?x06 | 9 | �?PASS |
...
| SYS-17 | SET_CONFIG McpBaudRate + 回读 | 0x03�?x02 | 3 | �?PASS |
| SYS-18 | SET_CONFIG 拒绝过低 McpBaudRate | 0x03 | 1 | �?PASS |
| SYS-19 | SET_CONFIG 拒绝过高 McpBaudRate | 0x03 | 1 | �?PASS |
| SYS-20 | McpBaudRate 端到端切�?重连 | 0x03�?x04�?x00�?x01 | 7 | �?PASS |
...
### 5.6 SYS-06: RESET + SYS_BOOT_EVENT �?端到端验�?
**测试流程**:
1. 发�?RESET (0x04), `ResetType=0x00`
2. 接收 RESET 响应: Status=`0x00` �?3. 设备软复位重�?(~500ms)
4. 接收 SYS_BOOT_EVENT 事件�?
**SYS_BOOT_EVENT 事件帧验�?(实际�?**:
...
### 5.7 SYS-07: 无效命令�?(cmd=0x07)

| 项目 | 预期 | 实际 |
|:---|:---|:---|
| 响应 Status | `0x06` (ERR_NOT_SUPPORT) | �?0x06 |

### 5.8 SYS-09: SYS_BOOT_EVENT 结构完整性验�?
...
### 5.11 SYS-12: GET_CONFIG �?FlowControlEnable (默认�?
...
### 5.14 SYS-15: SET_CONFIG �?拒绝写入只读 Key (旧策�?

> **注意**: �?`key >= UBCP_CFGKEY_READONLY_MASK (0x10)` 策略会将 `McpBaudRate (0x12)` 也误判为只读。当前版本已改为显式枚举只读 Key (`UART_CHANNEL_COUNT=0x10`, `CAN_CHANNEL_COUNT=0x11`)�?
| 步骤 | 操作 | 预期 | 实际 |
|:---|:---|:---|:---|
...
| 1 | Status | 0x00 | �?0x00 |
| 2 | ValueLen | 4 | �?4 |
| 3 | McpBaudRate (u32) | 非零 (编译默认值或 NVS 存储�? | �?115200 |
...
### 5.19 SYS-20: McpBaudRate 端到�?�?SET_CONFIG �?RESET �?新波特率重连

**测试流程**:
1. SET_CONFIG(ConfigKey=0x12, Value=230400) �?Status=0x00 �?2. RESET(0x00) 软复�?�?设备重启
3. �?230400 bps 重新打开 COM4
4. PING �?Status=0x00 �?5. GET_INFO �?ModelID="HXB1", ProtoVersion=0x02 �?6. SET_CONFIG 恢复 115200 �?Status=0x00 �?7. 软复位后�?115200 bps 重连 �?PING Status=0x00 �?
| 步骤 | 操作 | 预期 | 实际 |
|:---|:---|:---|:---|
...
| 7 | 复位�?115200 重连 PING | 0x00 | �?0x00 |

---

## 6. 新增 ConfigKey 验证

根据协议文档 `files/design/protocol/03-System.md#系统全局配置�?0x00-常用-ConfigKey-定义`�?
| ConfigKey | 参数名称 | 类型 | 预期�?| 实测�?| 读写属�?| 状�?|
|:---|:---|:---|:---|:---|:---|:---|
...
| 0x12 | McpBaudRate | u32 | 921600 (编译默认�? | 115200 (NVS 存储�? | 读写 | �?|

只读保护验证:
- �?`UartChannelCount` (0x10) 执行 SET_CONFIG 返回 `ERR_PERMISSION (0x0C)` �?- �?`CanChannelCount` (0x11) 执行 SET_CONFIG 返回 `ERR_PERMISSION (0x0C)` �?(代码审查)
- �?`McpBaudRate` (0x12) 执行 SET_CONFIG 正常写入 �?(证实仅显式枚�?Key 为只�?

---

## 7. ResetReason 映射验证

| ESP-IDF 常量 | UBCP �?| 名称 | 验证状�?|
|:---|:---|:---|:---|
...
| `ESP_RST_INT_WDT` | `0x06` | INT_WDT_RESET | 破坏�?|
| `ESP_RST_TASK_WDT` | `0x07` | TASK_WDT_RESET | 破坏�?|
| `ESP_RST_WDT` | `0x08` | WDT_RESET | 破坏�?|
| `ESP_RST_BROWNOUT` | `0x0D` | BROWNOUT_RESET | 硬件依赖 |
| `ESP_RST_PANIC` | `0x0E` | PANIC_RESET | 破坏�?|
| 其他 | `0xFF` | UNKNOWN | 兜底�?|

> �?SW_RESET 已验证外，其余映射均为一对一 switch-case 静态映射，逻辑正确性由代码审查保证�?
---

## 8. 代码变更清单

### 8.1 固件变更 (v0.1.0, 2026-07-22: MCP 波特率配�?

| 文件 | 变更 |
|:---|:---|
| `main/protocol/ubcp_def.h` | 新增 `UBCP_CFGKEY_MCP_BAUD_RATE` (0x12) |
| `main/transport/mcp_transport.h` | `init()` 签名改为 `mcp_transport_init(uint32_t baud_rate)`，传 0 使用编译默认�?|
| `main/transport/mcp_transport.c` | `uart_hw_init()` 接收动态波特率参数 |
| `main/modules/mod_system.h` | 新增 `mod_system_get_mcp_baud_rate()` 公开获取�?|
| `main/modules/mod_system.c` | `system_init()` �?NVS 加载 McpBaudRate (9600~5000000 校验)；`handle_get_config()` 返回 u32 大端波特率；`handle_set_config()` 校验范围 + NVS 写入 + commit；只读检查改为显式枚�?(`UART_CHANNEL_COUNT`, `CAN_CHANNEL_COUNT`) |
| `main/main.c` | 启动时调�?`mod_system_get_mcp_baud_rate()` 传入 `mcp_transport_init()` |
...
| `main/modules/mod_system.c` | 实现 `handle_get_config()` (0x02): 读取 DeviceName/HeartbeatInterval/FlowControlEnable/UartChannelCount/CanChannelCount；实�?`handle_set_config()` (0x03): 写入 DeviceName/HeartbeatInterval/FlowControlEnable (只读检�? |
...
| 文件 | 变更 |
|:---|:---|
| `script/test/test_mcp_baud.py` | **新增** �?MCP 波特率自动化测试脚本 (SYS-16~SYS-20)，支�?`--e2e` 端到端切换测�?|

### 8.6 设计文档变更

| 文件 | 变更 |
|:---|:---|
| `files/design/protocol/03-System.md` | 新增 ConfigKey=0x12 (McpBaudRate) 协议定义、使用说�?|
| `files/design/test/01-System-Tests.md` | 新增 SYS-16~SYS-20 测试用例 |
| `files/design/test-report/System-Test-Report.md` | **本报�?* �?更新�?v0.1.0 MCP 波特率配置完整测试记�?|

### 8.4 测试脚本变更 (2026-07-10)

| 文件 | 变更 |
|:---|:---|
| `script/test/test_system.py` | 新增 SYS-04/05/10/11/12/13/14/15 测试函数；新增辅助函�?`do_get_config()`/`do_set_config()` |

### 8.3 设计文档变更 (2 文件)

| 文件 | 变更 |
|:---|:---|
| `files/design/test/01-System-Tests.md` | SYS-04/SYS-05 更新为已实现；新�?SYS-10/11/12/13/14/15 配置测试用例 |
| `files/design/test-report/System-Test-Report.md` | **本报�?* �?更新为包�?GET_CONFIG/SET_CONFIG 的完整测试记�?|

---

## 9. 结论

系统管理模块 8 个已实现命令 (PING, GET_INFO, GET_CONFIG, SET_CONFIG, RESET, FLOW_CONTROL, SYS_BOOT_EVENT, GET_TOPOLOGY) 经过 17 项测试用例�?*81 项断言全部通过**�? 失败 0 跳过�?
...
- �?UartChannelCount 只读，�?1
- �?CanChannelCount 只读，�?2
- �?只读 Key 写入被拒�?(ERR_PERMISSION)
- �?写入后回读一致性验�?(DeviceName + HeartbeatInterval)

### 9.2 MCP 波特率配�?(v0.1.0 新增, 2026-07-22)
- �?McpBaudRate (ConfigKey=0x12) 可读 (GET_CONFIG 返回 u32 大端)
- �?McpBaudRate 可写 (SET_CONFIG �?NVS 持久�?
- �?波特率有效范围校�? 拒绝 < 9600 �?> 5000000 (ERR_PARAM)
- �?写入后回读一致性验�?(230400)
- �?端到�? SET_CONFIG(230400) �?RESET �?230400 重连 �?PING �?GET_INFO �?恢复 115200
- �?NVS 持久�? 软复位后波特率保持跨重启一�?
**固件编译、烧录、测试全流程验证通过�?*
