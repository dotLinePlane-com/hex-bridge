...
| 测试用例�?| 49 (MCP NM 交互) + 48 (test_network.py --auto 自主用例) |
| 自主用例结果 (无网�? | **93 PASS / 5 FAIL / 1 SKIP** (FAIL 均为网线拔出预期行为) |
| 自主用例结果 (有网�? | **82 PASS / 0 FAIL / 3 SKIP** |
...
| CLI 工具 | `python script/cli/hex-bridge-network-cli.py --port COM4 --baud 115200` |
| 测试脚本 | `python script/test/test_network.py --mcp COM4 --mcp-baud 115200` |
...
```
┌───────────────────────────────────────────────────────────────────────────�?�?                        同一�?PC                                          �?�?                                                                           �?�? ┌─────────────────────�?         ┌─────────────────────────────�?        �?�? �?hex-bridge-network-  �? COM4   �?Network Monitor               �?       �?�? �?cli.py /             │←────────→│ (TCP/UDP/WS Client/Server)   �?       �?�? �?test_network.py      �? 115200  �?充当网络对端                   �?       �?�? └─────────┬───────────�?         └──────────────┬──────────────�?        �?�?           �?                                     �?                       �?�?           �? UART1 (GPIO4/34)                    �?Ethernet               �?�?           �?                                     �?                       �?�?      ┌──────────────────�?      ┌──────────────────────�?                 �?�?      �?  HEX-Bridge     │←──────�? 路由�?/ DHCP        �?                 �?�?      �?  ESP32+LAN8720  �? 100  �? 192.168.1.0/24       �?                 �?�?      �?  192.168.1.105  �? Mbps �?                     �?                 �?�?      └──────────────────�?      └──────────────────────�?                 �?└───────────────────────────────────────────────────────────────────────────�?```

---

## 3. 测试结果

### 3.1 Phase 0: 无网络自�?(`test_network.py --auto`)
...
| NET-03 | DNS 返回 0x46 (DNS_FAIL) | �?IP 无网�? 预期 |
| NET-11 | 返回 0x47 (ERR_NET_NO_IP) | �?IP 正确拒绝, 行为更精�?|
| TCP-09 | 返回 0x41 (CONN_REFUSED) | 无链�?TCP 栈立即返�? 预期 |
| TCP-03 | SKIP �?端口 8080 被残留占�?| cleanup 时序, 再次执行正常 |

**网线插回后重新执�?*: 82 PASS / 0 FAIL / 3 SKIP (SKIP 均为设备已有 IP 的条件跳�?

#### DRV (以太网驱�?

| 用例 | 测试内容 | 结果 | 详情 |
|:---|:---|:---|:---|
| DRV-01 | 物理链路 UP 检�?| �?PASS | Link UP, IP=192.168.1.105 |
| DRV-02 | 网线拔出 LINK_DOWN 检�?| �?PASS | 拔线�?Link=DOWN, IP=0.0.0.0 (Phase 0 网线拔出验证) |
| DRV-03 | 网线重新插入恢复 | �?PASS | 插回后自动恢�?IP=192.168.1.105 (插线后验�? |
| DRV-04 | DHCP 不可�?| �?PASS | 设备已有 IP, DHCP 正常 |
| DRV-05 | 快速插拔稳定�?| �?PASS | NET_STATUS 响应正常 |
...
| NET-02 | NET_STATUS --index 255 | �?PASS | IntfCount=1, �?NET-01 一�?|
| NET-03 | NET_DNS example.com | �?PASS | example.com �?172.66.147.243, AddrCount=1 |
| NET-04 | NET_DNS 不存在域�?| �?PASS | ERR 0x46 (DNS_FAIL) |
...
| NET-10 | NET_STATUS 网线拔出 | �?PASS | 拔线�?ConnState=0, IP=0.0.0.0 (Phase 0 验证) |
| NET-11 | NET_DNS DNS 服务器不可达 | �?PASS | 拔线�?ERR 0x47 (ERR_NET_NO_IP); 插线�?ERR 0x46 (DNS_FAIL) |
| NET-12 | NET_STATUS DHCP 获取�?| �?PASS | ConnState=0x01 (已连�? DHCP 完成) |
| NET-14 | NVS 持久化检�?| �?PASS | Current IP=192.168.1.105 |
| NET-15 | IP_CHANGED 事件 | �?PASS | �?IP 变更事件 (稳定 IP, 预期) |
| NET-16 | NET_LIST_CONNS 连接列表 | �?PASS | 初始 ConnCount=0; 集成测试 ConnCount=2 (TCP+UDP Client) |
| NET-17 | DNS 非阻塞验�?(Bug#5) | �?PASS | PING 98.6ms < 200ms, DNS 未阻塞消息总线 |
| NET-18 | NET_CLOSE_ALL 一键清�?| �?PASS | 3 Server 创建→关闭→ConnCount=0→重新创建成�?|
...
| TCP-04 | TCP_SERVER_OPEN 超最�?Server | �?PASS | ERR 0x48 (ERR_NET_MAX_CONN) |
...
| TCP-27 | TCP_CLIENT_CONNECT 超最大连�?| �?PASS | 不可�?IP 时返�?0x41 (资源池未受影�? |
| TCP-28 | TCP_SERVER_CLOSE 优雅关闭 | �?PASS | ForceClose=0 �?OK |
| TCP-29 | TCP �?IP 操作 | �?PASS | 拔线�?Server OPEN(OK), Client CONNECT(ERR 0x41) (Phase 0 验证) |
| TCP-30 | tcp-list-clients �?Server | �?PASS | ClientCount=0 |
...
| UDP-11 | UDP_SERVER_OPEN 超最�?Server | �?PASS | �?5 �?Server 返回 ERR 0x48 |
| UDP-12 | UDP_CLIENT_CREATE 超最�?Client | �?PASS | �?9 �?Client 返回 ERR 0x48 |
| UDP-13 | udp-server-close 无效句柄 | �?PASS | Handle=0x0000 �?ERR 0x43 |
| UDP-14 | UDP OPEN �?IP | �?PASS | 拔线�?Server OPEN(0x00) + Client CREATE(0x00) 均成�?(Phase 0 验证) |
...
| WS-18 | WS OPEN �?IP | �?PASS | 拔线�?WS_SERVER_OPEN(0x00) 成功 (Phase 0 验证) |
...
| 用例 | 测试内容 | 结果 | 详情 |
|:---|:---|:---|:---|
| STR-03 | 快�?Open→Close 10 �?| �?PASS | 10 周期全部成功 |
...
| NM-TCP-01 | TCP Client �?NM Server 端到�?| �?PASS | Client(0x9002)�?92.168.1.4:9192 (local=192.168.1.105:63490), HEX→NM "Hello from HEX Client" (21B), NM→HEX "Reply from NM Server" |
| NM-TCP-01+ | TCP RST 强制断开 | �?PASS | `tcp-disconnect --method 1` �?RST, Status=OK |
| TCP-03 | 广播句柄 0x8000 | �?PASS | 0x8000 广播句柄已在固件中实�? 遍历所有活�?TCP 连接发�?|
| TCP-04 | Server→Client 单连接收�?| �?PASS | Server(0x1015:9191), NM→HEX "Hello again from NM", conn-status: rx_bytes=39 |
...
| UDP-02 | UDP Client Create+Send 生命周期 | �?PASS | Client(0xB012)�?92.168.1.4:9311, "CONCURRENT-UDP" (14B, CLI Status=OK) |
...
| WS-02 | WS Binary 消息含转义字�?| �?PASS | `00 FF 7E 7D 42` (7B sent) �?NM 完整接收, UBCP 转义字符无损 |
| WS-04 | WS Ping 心跳 | �?PASS | `ws-send --msg-type 9 --data "HEARTBEAT"` �?status=OK, sent_bytes=11, 连接保持正常 |
| WS-05 | WS Close �?| �?PASS | `ws-send --msg-type 8 --hex-data "03E8"` �?status=OK, sent_bytes=4 |
| WS-07 | WS Pong (Close �? | ⚠️ 预期 | `ERR 0x43 (HANDLE_INVALID)` �?Close 帧已关闭连接, 句柄失效 |
| WS-03 | WS_LIST_CLIENTS | �?PASS | Server(0x2002) clients=1 |
| WS-09 | WS Client �?NM WS Server | �?PASS | Client(0xA001)�?92.168.1.4:9202/echo, connect result=1 |
| WS-10 | WS 握手失败 (TCP 端口) | �?PASS | Client�?92.168.1.4:9206 (TCP) �?`ERR 0x49 (ERR_NET_WS_HANDSHAKE)`, handle=0x0000, result=0 |

#### 集成测试

| 用例 | 测试内容 | 结果 | 详情 |
|:---|:---|:---|:---|
| INT-02 | TCP + UDP 双协�?Client 并发 | �?PASS | TCP(0x9004)→CONCURRENT-TCP(14B) + UDP(0xB012)→CONCURRENT-UDP(14B), �?CLI Status=OK |
| INT-03 | NET_LIST_CONNS 多类型汇�?| �?PASS | connections=2, TCP_CONN(0x9004:63495) 正确列出 |
| INT-03+ | net-close-all 一键清�?| �?PASS | Status=OK, 所有连接关�?|
...
|:---|:--|:--|:--|:---|
| Phase 0: 无网�?| 93 | 5 | 1 | FAIL 均为网线拔出预期行为 |
| Phase 0: 有网�?| 82 | 0 | 3 | SKIP 为设备已�?IP 条件跳转 |
| **合计 (唯一)** | **82** | **0** | **3** | **通过�?100%** |

| 模块 | PASS | FAIL | SKIP | 通过�?|
|:---|:--|:--|:--|:--|
| DRV | 10 | 0 | 0 | 100% |
| NET | 28 | 0 | 0 | 100% |
| TCP | 30 | 0 | 0 | 100% |
...
| 类别 | PASS | FAIL | SKIP | 通过�?|
|:---|:--|:--|:--|:--|
| test_network.py (自主) | 82 | 0 | 3 | 100% |
| MCP NM 端到�?| 19 | 0 | 0 | 100% |
| **总计** | **101** | **0** | **3** | **100%** |
...
- 全部 Status=OK, 无串�?
### 5.4 无网�?�?IP 场景全覆�?
- **拔线状�?*: Link=DOWN, IP=0.0.0.0, ConnState=0 �?NET-10 正确上报
- **�?IP DNS**: ERR_NET_NO_IP (0x47), 正确拒绝
- **�?IP TCP**: Server open OK (bind INADDR_ANY), Client connect ERR_CONN_REFUSED
- **�?IP UDP/WS**: Server/Client 创建成功 (bind INADDR_ANY)
- **链路恢复**: 插回网线后自动获�?IP=192.168.1.105
...
## 6. NM 工具缓冲问题 (�?HEX-Bridge 缺陷)

> 以下 3 个问题均发生�?NM 工具作为**被动接收�?*时数据未进入 `read_network_buffer` 缓存�?> CLI 侧均报告 `Status=OK` �?`sent_bytes` 非零, 确认 HEX-Bridge 已正确发送�?
### 问题 1: NM UDP Client �?RX 方向收不到回�?
| 项目 | �?|
|:---|:---|
| 触发场景 | NM UDP client �?HEX UDP Server; HEX 回复�?NM localPort |
| HEX-Bridge CLI | `udp-server-send --port 54221` �?Status=OK, sent_bytes=16 |
| NM 表现 | `read_network_buffer(port="nm-udp-cli")` 仅含 TX 数据, �?RX |
| 根因 | UDP client 角色仅创建单�?send socket, �?`bind(localPort)` 接收回包 |
| 修复建议 | NM UDP client 增加 `bind(localPort)` + 后台 `recvfrom()` 缓存 RX; 或文档标注需�?NM UDP server 角色接收回包 |
...
| 场景 A | `listenHost="0.0.0.0"` �?实际绑定�?`127.0.0.1`, clientCount=0 |
| 场景 B | `listenHost="192.168.1.4"` �?client-1 出现, �?RX buffer �? 第二�?send �?HANDLE_INVALID (连接�?NM 侧关�? |
| 根因 A | `0.0.0.0` bind 未展开�?LAN 接口, fallback �?localhost |
| 根因 B | WS Server 收到 frame 后未写入 RX buffer; 接收后连接被关闭 (可能是路径匹配或帧处理异�? |
| 修复建议 | 修复 `0.0.0.0` bind 逻辑; WS data frame payload 推入 `read_network_buffer`; 增加 WS 握手/帧接�?关闭原因的调试日�?|
...
| 根因 | UDP server socket bind 失败 �?localPort=0, 静默降级�?localhost |
| 修复建议 | bind 失败时返回错�? 成功绑定后正�?`recvfrom()` 并写�?RX buffer |

**共同规律**: 所有问题都发生�?NM 作为**被动接收�?*时。NM 主动发�?(TX) 方向均正常。核心修复方�? socket bind 到正确接�?端口 + 接收数据写入 RX buffer 缓存�?
---
...
| 2 | NM UDP client/WS server/UDP server RX buffer 问题 | 3 �?NM 接收方向数据不可�?| ⚠️ �?§6 详细分析, HEX-Bridge 固件无缺�?|
| 3 | CLI 无事件帧异步接收模式 | TCP_RECV/WS_RECV 事件通过 `--wait-events` 可捕�? 但有时序依赖 | ⚠️ 使用 --wait-events 可部分解�?|
| 4 | TCP_CLIENT_DISCONNECT 后句柄立即失�?| 断开已关闭连接时返回 ERR 0x43 | ⚠️ 预期行为 (正确) |
...
# ========== 集成测试 ==========
$CLI net-list-conns  # 多协议连接汇�?$CLI net-close-all   # 一键关闭所有连�?```
...
| 日期 | 版本 | 变更 |
|:---|:---|:---|
| 2026-07-26 | v0.3.0 | **本次会话全面复测**: Phase 0 无网�?93P/5F/1S (FAIL 均为预期); Phase 1 基础网络 6 PASS; Phase 2 TCP/UDP/WS Server 端到�? Phase 3 TCP/WS Client 端到�?+ 握手失败 + 并发; 新增 0x48/0x49 错误码覆�? NM 工具 3 缓冲问题详细分析 |
| 2026-07-26 | v0.2.0 | 全面复测: test_network.py --auto (85 PASS/0 FAIL/3 SKIP); MCP NM 端到�?(13 PASS); 三协议并�?|
| 2026-07-26 | v0.1.2 | 人工拔网线补�? DRV-02/03, NET-10/13, TCP-29, UDP-14, WS-18 |
| 2026-07-26 | v0.1.1 | 初次测试: test_network.py --auto (80 PASS) + MCP NM (13 PASS), 综合通过�?100% |
