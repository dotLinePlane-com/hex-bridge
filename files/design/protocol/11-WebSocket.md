...
| (4+L) | SubProtoLen | u8 | 子协议列表长度 (S, 0 = 无子协议), **当前版本保留，固件未实现** |
| (5+L)...(5+L+S-1) | SubProtocol | str(S) | 子协议列表 (逗号分隔), **当前版本保留，固件未实现** |

> **v1.0 修正**：可选字段使用明确的长度前缀，消除了"Byte 之后"的歧义。当 SubProtoLen=0 时，子协议字段不存在。**当前版本固件忽略子协议数据，该字段保留用于未来扩展。**
