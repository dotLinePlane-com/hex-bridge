...
> **注意**：
> - 软复位和硬件复位后设备需要重新初始化，主机应等待设备发送 PING 响应后再发送其他命令。
> - **ResetType=0x00 (软复位)** 在调用 `esp_restart()` 前会依次执行各模块的 `stop()` 钩子（反向遍历注册顺序）。`stop()` 负责关闭所有 TCP/UDP/WS socket、停止后台 select 任务、清理网络连接资源，确保对端收到有序的 RST/FIN 而非静默断开。
